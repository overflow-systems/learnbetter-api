...
class MentoradoController {
  async buscarUsuario(request, response) {
    const usuarios = await knex
      .select('*')
      .from('mentores')
      .unionAll(knex.select('*').from('mentores'));
  ...
  async criarUsuario(request, response) {
    ...
    const erros: VerificarEmailCelularInterface = await VerificarEmailCelular(
      tipo,
  ...
  async login(request, response) {
    ...
    if (bcrypt.compareSync(usuario.senha, senha))
      return response.json({ status: 200, mensagem: 'OK' });
